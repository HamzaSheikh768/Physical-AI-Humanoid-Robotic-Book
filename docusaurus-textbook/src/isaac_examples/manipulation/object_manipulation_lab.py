...
class ObjectManipulationLab(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.manipulation_status_pub = self.create_publisher(Header, 'manipulation_status', 10)
        self.lab_status_pub = self.create_publisher(Header, 'lab_status', 10)
        self.visualization_pub = self.create_publisher(MarkerArray, 'lab_visualization', 10) if self.enable_visualization else None
    ...
    def object_poses_callback(self, pose_array_msg):
        ...
        """
        self.get_logger().info(f'Received {len(pose_array_msg.poses)} object poses for manipulation planning')

        # Store detected objects
        self.detected_objects = pose_array_msg.poses

        # Filter objects based on confidence and criteria
        self.objects_to_manipulate = self.filter_objects_for_manipulation(self.detected_objects)
    ...
    def filter_objects_for_manipulation(self, objects: List[Pose]) -> List[Pose]:
        ...
        for i, obj_pose in enumerate(objects):
            ...
            dist = np.sqrt(obj_pose.position.x**2 + obj_pose.position.y**2 + obj_pose.position.z**2)
    ...
    def plan_manipulations(self):
        ...
        with self.manipulation_lock:
            ...
            for obj_pose in self.objects_to_manipulate:
                ...
                grasp_plan = self.grasp_planner.plan_grasp(obj_pose, object_info, robot_info)
                ...
                if grasp_plan:
                    ...
                    self.get_logger().warn(f'Could not plan grasp for object at '
                                         f'({obj_pose.position.x:.2f}, {obj_pose.position.y:.2f}, {obj_pose.position.z:.2f})')
        ...
        with self.manipulation_lock:
            while self.manipulation_queue:
                ...
                if success:
                    ...
                    self.get_logger().info(f'Manipulation {self.manipulation_completed} completed successfully')

                    # Publish manipulation status
                    status_header = Header()
                    status_header.stamp = self.get_clock().now().to_msg()
                    status_header.frame_id = f'manipulation_{self.manipulation_completed}_success'
                    self.manipulation_status_pub.publish(status_header)
                else:
                    self.get_logger().error(f'Manipulation {self.manipulation_completed + 1} failed')
    ...
    def calculate_place_pose(self, manipulation_count: int) -> Pose:
        ...
        place_pose.position.y = grid_center_y + (col - (grid_size-1)/2) * grid_spacing
